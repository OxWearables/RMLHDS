...
class SetUp:
    ...
    def __init__(self, root):
        ...
        Button(self.image_path_frame, text="Choose", command=self.choose_image_path).grid(row=0, column=1, sticky='e')

        # number (N) of images to display
        ttk.Label(self.window, text="Number (N) of images to display:").grid(row=1, column=0)
        ...
        ttk.Label(self.window, text="Which of the N images will you annotate?").grid(row=2, column=0)
        # add combo box to choose between 1 to 5 images
        ...
        Button(self.schema_path_frame, text="Choose", command=self.choose_schema_path).grid(row=0, column=1, sticky='e')
        ...
        Button(self.save_path_frame, text="Choose", command=self.choose_save_path).grid(row=0, column=1, sticky='e')
        ...
        Button(self.start_button_frame, text="Start", command=self.get_selection_and_start).grid(row=5, column=0, sticky='e')
    ...
    def get_selection_and_start(self):
        ...
        if self.image_index >= self.n_display_images:
            messagebox.showinfo(message="Please choose an image index less than the number of images to display")
            return False
        
        # check that the image directory conforms to the format
        if not check_img_root_dir(self.img_root_dir):
            messagebox.showinfo(message="Current image folder is either empty, or is incorrectly formated.")
            return False
